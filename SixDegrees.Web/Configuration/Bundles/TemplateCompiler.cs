...
Options:
  ...
  -r, --root           Template root. Base value that will be stripped from template names.  [string]
        */
        ...
        private const string ArgumentsFormat = "{0} -f {1} -a {2} -m {3} ";
        ...
        public string Compile()
        {
            ...
            this.Log().Info("Compiling handlebars templates: {0} {1}", compileCommand.FileName, compileCommand.Arguments);
        ...
        private string BuildArguments()
        {
            StringBuilder builder = new StringBuilder(
                string.Format(
                    ArgumentsFormat,
                    Path.Combine(_workingDirectory, "bin\\handlebars"),
                    _outputFileName,
                    _shouldExportAMD.ToString().ToLower(),
                    _shouldMinify.ToString().ToLower()));
